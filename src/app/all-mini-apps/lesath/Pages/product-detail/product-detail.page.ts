...
export class ProductDetailPage implements OnInit {
  ...
  @ViewChild('topSlider', { static: false }) slides: IonSlides;
  ...
  getGroupProducts() {
    ...
    for (let value of this.product.grouped_products) {
      count++;
      this.config.getWoo("products/" + value).then(data => {
  ...
  updateProductDetail(p) {
    ...
    this.product.images.forEach((element, index) => {
      if (p.images[0].src == element.src) ind = index;
    });
    if (ind != 0) {
  ...
  // <!-- 2.0 updates -->
  getRelatedItems() {
    this.config.getWoo("products/?include=" + this.product.related_ids).then((data: any) => {
  ...
  getRatingValue(value) {
    let val = value;
    let result = "star-outline";
    if (val == 1) {
      if (this.ratingStarsValue > 0 && this.ratingStarsValue <= 10)
        result = "star-half";
      else if (this.ratingStarsValue >= 10)
        result = "star";
    }
    else if (val == 2) {
      if (this.ratingStarsValue > 20 && this.ratingStarsValue <= 30)
        result = "star-half";
      else if (this.ratingStarsValue >= 30)
        result = "star";
    }
    else if (val == 3) {
      if (this.ratingStarsValue > 40 && this.ratingStarsValue <= 50)
        result = "star-half";
      else if (this.ratingStarsValue >= 50)
        result = "star";
    }
    else if (val == 4) {
      if (this.ratingStarsValue > 60 && this.ratingStarsValue <= 70)
        result = "star-half";
      else if (this.ratingStarsValue >= 70)
        result = "star";
    }

    else if (val == 5) {
      if (this.ratingStarsValue > 80 && this.ratingStarsValue <= 90)
        result = "star-half";
      else if (this.ratingStarsValue >= 90)
        result = "star";
    }
    return result;
  }
  ...
  getProductImage(p) {
    if (p == 1) return "assets/woocommerce-placeholder.png";
